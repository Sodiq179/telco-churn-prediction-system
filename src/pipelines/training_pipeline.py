...
def get_active_model_name() -> str:
    return MODEL_CONFIG["active_model"]


def setup_mlflow_for_final_training(active_model: str) -> None:
    mlflow.set_tracking_uri(APP_CONFIG["mlruns_dir"])
    experiment_prefix = MODEL_CONFIG["tracking"]["experiment_prefix"]
    mlflow.set_experiment(f"{experiment_prefix}_{active_model}_final")
...
def main():
    ...
    setup_mlflow_for_final_training(active_model)

    with mlflow.start_run(run_name=f"final_{active_model}_model"):
        ...
        mlflow.log_artifact(METADATA_OUTPUT_PATH)
